...
const LoginPage: React.FC = () => {
  ...
  const handleGoogleSignIn = () => signIn("google", { callbackUrl: "/DashboardPage" });
  const handleFacebookSignIn = () => signIn("facebook", { callbackUrl: "/DashboardPage" });
    ...
    <div className="min-h-screen flex items-center justify-center px-4 sm:px-6 lg:px-8 relative pt-16">
      ...
      <div className="relative max-w-md w-full">
        ...
        <div className="bg-glass-bg backdrop-blur-lg border border-secondary/20 rounded-3xl p-8 shadow-xl">
          ...
          {error === "OAuthAccountNotLinked" && <div className="mb-6 p-4 bg-yellow-50 border-l-4 border-yellow-400 text-yellow-800 rounded">You’ve already signed up with a different method. Please sign in with email & password first, then reconnect Google in your account settings.</div>}
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            <div>
              ...
              <div className="relative">
                ...
                <input id="email" name="email" type="email" placeholder="Enter your email" value={formData.email} onChange={handleChange} className="w-full pl-12 pr-4 py-4 bg-baby-powder/50 border rounded-2xl text-text placeholder-text/50 focus:outline-none focus:ring-2 border-secondary/30 focus:border-primary focus:ring-primary/20" required />
              </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input id="password" name="password" type={showPassword ? "text" : "password"} placeholder="Enter your password" value={formData.password} onChange={handleChange} className="w-full pl-12 pr-12 py-4 bg-baby-powder/50 border rounded-2xl text-text placeholder-text/50 focus:outline-none focus:ring-2 border-secondary/30 focus:border-primary focus:ring-primary/20" required />
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-text/40 hover:text-text transition-colors">{showPassword ? <EyeOff size={20} /> : <Eye size={20} />}</button>
              </div>
            ...
            <button type="submit" disabled={isLoading} className={`group w-full flex items-center justify-center space-x-2 py-4 rounded-2xl font-semibold text-lg transition-all ${isLoading ? 'bg-text/20 text-text/40 cursor-not-allowed' : 'bg-gradient-to-r from-primary to-primary-light text-baby-powder hover:shadow-lg hover:scale-105'}`}>
              {isLoading ? <div className="w-6 h-6 border-2 border-text/20 border-t-text/40 rounded-full animate-spin"></div> : <><span>Sign In</span><ArrowRight size={20} className="group-hover:translate-x-1 transition-transform" /></>}
            </button>
          </form>
          ...
          <div className="grid grid-cols-2 gap-4">
            <button type="button" onClick={handleGoogleSignIn} className="flex items-center justify-center px-4 py-3 border border-secondary/30 rounded-2xl bg-baby-powder/50 hover:bg-baby-powder transition-colors"><img src="https://www.google.com/favicon.ico" alt="Google" className="w-5 h-5 mr-2" /><span className="text-sm font-medium text-text">Google</span></button>
            <button type="button" onClick={handleFacebookSignIn} className="flex items-center justify-center px-4 py-3 border border-secondary/30 rounded-2xl bg-baby-powder/50 hover:bg-baby-powder transition-colors"><img src="https://www.facebook.com/favicon.ico" alt="Facebook" className="w-5 h-5 mr-2" /><span className="text-sm font-medium text-text">Facebook</span></button>
          </div>
          ...
          <div className="mt-6 text-center">
            <button onClick={() => signOut({ callbackUrl: "/LoginPage" })} className="text-sm text-gray-600 underline hover:text-gray-800">Log out / Reset</button>
          </div>
        </div>
      </div>
    </div>
